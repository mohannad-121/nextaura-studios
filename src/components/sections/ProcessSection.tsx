      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {STEPS.map((step) => (
            <div
              key={step.num}
              className="group relative rounded-2xl bg-white border border-black/8 hover:border-[#FF6A1A]/40 p-8 flex flex-col justify-between transition-all duration-300 hover:shadow-[0_12px_30px_rgba(21,21,21,0.04)]"
            >
              <div>
                <div className="flex items-center justify-between pb-4 mb-4 border-b border-black/6">
                  <span className="font-mono text-xs font-bold px-2 py-0.5 rounded-sm bg-[#A90F24]/10 text-[#A90F24]">
                    {step.num}
                  </span>
              ...
              <div className="mt-8 pt-4 border-t border-black/5 flex items-center justify-between text-[11px] font-mono text-[#969490]">
                <span>PHASE {step.num}</span>
                <span className="w-1.5 h-1.5 rounded-full bg-[#FF6A1A] opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            </div>
          ))}
        </div>

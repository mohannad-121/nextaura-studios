        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {DOMAINS.map((domain, index) => {
            return (
              <div
                key={domain.id}
                className="group relative rounded-2xl bg-white border border-black/8 hover:border-[#FF6A1A]/40 p-8 sm:p-10 flex flex-col justify-between transition-all duration-500 hover:shadow-[0_20px_50px_rgba(21,21,21,0.06),0_0_25px_rgba(255,106,26,0.12)] overflow-hidden"
              >
                {/* Solar Radial Hover Light */}
                <div
                  className="pointer-events-none absolute -top-20 -right-20 w-64 h-64 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700"
                  style={{
                ...
                <div className="flex items-center justify-between mb-8 relative z-10">
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-xs font-bold text-[#FF6A1A] tracking-wider px-2 py-0.5 rounded-sm bg-[#FF6A1A]/10">
                      0{index + 1}
                    </span>
                  ...
                  <Link
                    href={domain.link}
                    className="w-9 h-9 rounded-full border border-black/10 bg-[#FAF9F6] flex items-center justify-center text-[#151515] group-hover:bg-[#A90F24] group-hover:text-white group-hover:border-[#A90F24] transition-all duration-300"
                ...
                <div className="relative w-full h-44 sm:h-52 rounded-xl bg-gradient-to-br from-[#F4F4F2] via-[#FFFDFC] to-[#ECECEA] border border-black/5 mb-8 flex items-center justify-center overflow-hidden">
                  {/* Visual: Mobile App */}
                  {domain.visualType === "app" && (
                    <div className="relative w-36 h-48 rounded-[20px] bg-white border-2 border-black/10 shadow-md p-3 flex flex-col justify-between group-hover:-translate-y-2 transition-transform duration-500">
                      <div className="w-10 h-1.5 rounded-full bg-black/15 mx-auto mb-2" />
                      <div className="space-y-2">
                        <div className="w-full h-8 rounded-lg bg-[#A90F24]/10 flex items-center px-2">
                          <span className="w-2 h-2 rounded-full bg-[#A90F24] animate-ping" />
                        </div>
                        <div className="w-3/4 h-2 rounded-full bg-black/10" />
                        <div className="w-1/2 h-2 rounded-full bg-black/10" />
                      </div>
                      <div className="flex justify-between items-center pt-2 border-t border-black/5">
                        <div className="w-4 h-4 rounded-full bg-[#FF6A1A]" />
                        <div className="w-8 h-2 rounded-full bg-black/10" />
                      </div>
                    </div>
                  )}
                  ...
                  {domain.visualType === "game" && (
                    <div className="relative w-full h-full flex items-center justify-center">
                      <div className="absolute w-28 h-28 rounded-full border border-dashed border-[#FF6A1A]/40 animate-orbit-slow" />
                      <div className="relative w-16 h-16 rounded-xl bg-gradient-to-tr from-[#A90F24] to-[#FF6A1A] shadow-lg flex items-center justify-center group-hover:scale-110 group-hover:rotate-12 transition-all duration-500">
                        <Sparkles className="w-7 h-7 text-white" />
                      </div>
                      <div className="absolute bottom-6 left-12 px-3 py-1 rounded-md bg-white border border-black/10 text-[10px] font-mono font-bold text-[#151515] shadow-xs">
                        120 FPS
                      </div>
                    </div>
                  )}
                  ...
                  {domain.visualType === "product" && (
                    <div className="relative w-48 h-32 rounded-lg bg-white border border-black/10 shadow-md p-3 flex flex-col justify-between group-hover:-translate-y-1 transition-transform duration-500">
                      <div className="flex items-center gap-1.5 pb-2 border-b border-black/5">
                        <div className="w-2 h-2 rounded-full bg-[#A90F24]" />
                        <div className="w-2 h-2 rounded-full bg-[#FF6A1A]" />
                        <div className="w-2 h-2 rounded-full bg-[#FFB24A]" />
                        <span className="text-[8px] font-mono text-[#969490] ml-auto">KROMA.APP</span>
                      </div>
                      <div className="grid grid-cols-3 gap-1.5 my-1">
                        <div className="h-10 rounded-sm bg-[#FAF9F6] border border-black/5" />
                        <div className="h-10 rounded-sm bg-[#A90F24]/10 border border-[#A90F24]/20" />
                        <div className="h-10 rounded-sm bg-[#FAF9F6] border border-black/5" />
                      </div>
                      <div className="w-full h-1.5 rounded-full bg-[#FF6A1A]/20" />
                    </div>
                  )}
                  ...
                  {domain.visualType === "experiment" && (
                    <div className="relative w-full h-full flex items-center justify-center">
                      <div className="absolute w-36 h-36 rounded-full bg-gradient-to-tr from-[#FF7424]/20 to-transparent animate-pulse-subtle" />
                      <svg viewBox="0 0 160 160" className="w-32 h-32 animate-orbit-reverse">
                        <polygon
                          ...
                          strokeDasharray="4 4"
                        />
                        <circle cx="80" cy="80" r="14" fill="#A90F24" />
                      </svg>
                    </div>
                  )}
                </div>
                ...
                <div className="relative z-10">
                  ...
                  <div className="mt-6 flex flex-wrap gap-2 pt-6 border-t border-black/5">
                    {domain.tags.map((tag) => (
                      <span
                        key={tag}
                        className="text-[11px] font-mono tracking-wide px-2.5 py-1 rounded-md bg-[#FAF9F6] text-[#151515] border border-black/5"
                      >
                        {tag}
                      </span>
                    ))}

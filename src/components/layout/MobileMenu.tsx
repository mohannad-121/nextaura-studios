    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-between">
        ...
        <button
          ...
          className="w-11 h-11 rounded-full border border-black/10 bg-white/80 flex items-center justify-center text-[#151515] hover:text-[#A90F24] hover:border-[#FF6A1A]/30 transition-all"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
        ...
        {NAV_LINKS.map((link, idx) => {
          ...
          return (
            ...
            >
              ...
              <ArrowUpRight className="w-6 h-6 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 text-[#FF6A1A]" />
            </Link>
          );
        })}
      </nav>
      ...
      <div className="relative z-10 pt-6 border-t border-black/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <AuraButton href="/contact" onClick={onClose} variant="primary" showArrow className="w-full sm:w-auto">
          Start a Project
        </AuraButton>
